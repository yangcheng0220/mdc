...
- **Reply; don't resolve.** Resolving hides the thread and with it the fresh reply. Resolving is normally the user's action after reading; only resolve on their explicit request. The one defined exception is a silent suggestion dismissal: acknowledge it briefly in the thread, then resolve as described below.
...
- **A dismissal passes the turn to the agent.** A rejected suggestion stays dismissed and the conversation stays open, so it appears in `list-pending` / `watch` with no actionable suggestion. Read the thread before acting: if the user added a reason after dismissing, engage with that reason and re-propose in the same thread; if the dismissal is silent, read it as "no, and nothing to add," acknowledge briefly, and resolve the thread. Never blind-guess a second version.
