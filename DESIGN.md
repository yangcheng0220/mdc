...
- **Menu icons follow what the item names.** An item naming a *kind of thing* reuses the tree's type glyph, so the menu and the row agree — New file / New drawing / New folder take `FileIcon` / `DrawingIcon` / `FolderIcon`. An item naming an *action* takes a dedicated action glyph: `Copy path` is the link glyph, not the file glyph, because `FileIcon` there would read as "a file" rather than "copy its path".
...
- **Start-truncating copied-value meta** — a toast echoing a copied path or filename ellipsizes from the **start**, keeping the identifying tail visible (`…/specs/doc-actions-menu/TECH.md`); absolute paths routinely outrun the toast, so this is the normal case, not an edge one. The recipe is `direction: rtl` on a `min-width: 0; overflow: hidden; white-space: nowrap; text-overflow: ellipsis` block, plus `text-align: left`, with the value itself wrapped in `<bdi>`. `direction: rtl` has two side effects that both need undoing, and both only show on values short enough *not* to overflow: it reorders a leading `/` to the end (`/tmp/a.md` paints as `tmp/a.md/`) — that's what the **required, not defensive** `<bdi>` prevents — and it right-aligns the line within its block, which `text-align: left` pins back. Only successful copy confirmations opt in — ordinary prose toasts keep their normal wrapping and direction. When such a meta line carries a trailing size/status suffix, keep the suffix in a non-shrinking LTR span and start-ellipsize only the filename, so truncation never eats the size to preserve the name.
